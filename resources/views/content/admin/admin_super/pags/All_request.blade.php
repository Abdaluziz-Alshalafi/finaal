                        ...
                        <td>
                            <div class="research-info">
                                {{-- @if($item->topics->isNotEmpty()) --}}
                                    <div class="research-title">{{ $item->pluck('name') ?? 'لا يوجد عنوان' }}</div>
                                    {{-- <div class="research-description">{{ $item->first()->describtion1 ?? 'لا يوجد وصف' }}</div> --}}
                                {{-- @else --}}
                            ...
                            <div class="student-list">
                                {{-- @forelse($item->requests as $request) --}}
                                    {{-- <div class="student-item">
                                        <div class="avatar">{{ substr($request->name ?? 'غ', 0, 1) }}</div>
                                        <div>
                                            <div class="person-name">{{ $request->name ?? 'غير معروف' }}</div>
                                            <div class="person-info">{{ $request->id ?? 'غير معروف' }}</div>
                                        </div>
                                    </div> --}}
                                @empty
                                ...
                                {{-- @forelse($item->projectSupervisors as $supervisor)
                                    <div class="supervisor-item">
                                        ...
                                        <div>
                                            ...
                                            <div class="person-info">{{ $supervisor->teacher->depart->depart_name ?? 'غير معروف' }}</div>
                                        </div>
                                    </div>
                                @empty
        ...
        @forelse($team as $item)
            <div class="research-card" data-id="{{ $item->id }}">
                <div class="card-header">
                    {{-- @if($item->topics->isNotEmpty()) --}}
                        <div class="card-title">{{ $item->first()->name ?? 'لا يوجد عنوان' }}</div>
                        {{-- <div class="card-description">{{ $item->first()->describtion1 ?? 'لا يوجد وصف' }}</div> --}}
                    {{-- @else
                ...
                <div class="card-body">
                    ...
                    <div class="card-section">
                        ...
                        <div class="supervisor-list">
                            @forelse($item as $supervisor)
                                <div class="supervisor-item">
                                    <div class="avatar supervisor">{{ substr($supervisor->name ?? 'غ', 0, 1) }}</div>
                                    <div>
                                        <div class="person-name">{{ $supervisor->name ?? 'غير معروف' }}</div>
                                        {{-- <div class="person-info">{{ $supervisor->teacher->depart->depart_name ?? 'غير معروف' }}</div> --}}
                                    </div>
                                </div>
                            @empty
            ...
            <div class="modal-body">
                <form id="researchForm">
                    ...
                    <div class="form-group">
                        <label class="form-label" for="name">عنوان البحث</label>
                        <input type="text" class="form-input" id="name" name="name" placeholder="أدخل عنوان البحث" required>
                        <div class="error-message" id="titleError"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="describtion1">وصف البحث</label>
                        <textarea class="form-input" id="describtion1" name="describtion1" rows="4" placeholder="أدخل وصف البحث" required></textarea>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.viewResearch = function(id) {
        ...
        window.location.href = `/superadmin/University/${id}`;
    };
    ...
    window.editResearch = function(id) {
        ...
        fetch(`/superadmin/University/${id}/edit`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.topics && data.topics.length > 0) {
                    document.getElementById('name').value = data.topics[0].name || '';
                    // document.getElementById('describtion1').value = data.topics[0].describtion1 || '';
                }
    ...
    function confirmDelete() {
        ...
        fetch(`/superadmin/University/${id}`, {
            method: 'DELETE',
            headers: {
    ...
    function saveResearch() {
        ...
        const url = id ? `/superadmin/University/${id}` : '/superadmin/University';
    ...
    // عرض رسالة إشعار
    function showToast(name, message, type = 'success') {
