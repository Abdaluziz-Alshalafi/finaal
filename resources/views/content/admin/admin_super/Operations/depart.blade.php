...
@section('content')
    ...
    <div class="container">
        <div class="dashboard-header">
            <div>
                {{-- <h1 class="dashboard-title">لوحة إدارة التخصصات</h1> --}}
                <p class="dashboard-subtitle">إدارة وتتبع بيانات التخصصات بسهولة</p>
            </div>
        ...
        <div class="stats-container">
            <div class="stat-card">
                ...
                <div class="stat-info">
                    ...
                    <div class="stat-label">إجمالي التخصصات</div>
                </div>
            </div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-info">
                    ...
                    <div class="stat-label">تخصصات نشطات</div>
                </div>
            </div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-info">
                    ...
                    <div class="stat-label"> تخصصات غير نشطات</div>
                </div>
            </div>
        ...
        <div class="table-header">
            <h2 class="table-title">قائمة التخصصات</h2>

            <div class="search-container">
                <input type="text" class="search-input" id="searchInput" placeholder="البحث عن تخصص...">
        ...
        <div class="table-container desktop-table-container">
            <table class="desktop-table">
                ...
                <td>{{ $depart->id }}</td>
                        ...
                        <td>
                            <div class="employee-info">
                                <div class="employee-details">
                                    <div class="employee-name">{{ $depart->depart_name }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                            ...
                            <div class="employee-info">
                                <div class="employee-details">
                                    {{-- <div class="employee-number">{{ $depart->id_coll }} --}}
                                        <div class="employee-number">{{ $depart->college->coll_name ?? 'غير معروف'  }}
                        ...
                        <td class="actions">
                            ...
                            <button class="action-btn edit tooltip" style="font-size:15px;" onclick="editEmployee( {{ $depart->id }},
                               '{{ $depart->depart_name }}','{{ $depart->id_coll }}')">
            ...
            <div id="emptyStateDesktop" class="empty-state" style="display: none;">
                ...
                <h3 class="empty-title">لا يوجد تخصصات</h3>
                <p class="empty-description">لم يتم العثور على أي موظفين. يمكنك إضافة موظف جديد بالنقر على زر "                إضافة تخصصات جديد".</p>
                <button class="btn btn-primary" onclick="openAddModal()">
                    <i class="fas fa-plus"></i>
                    إضافة تخصص جديد
                </button>
            ...
            <div class="employee-card">
                        <div class="card-header">
                            ...
                            <div class="card-title">
                                <div class="card-name">{{ $depart->depart_name }} </div>
                            </div>

                        </div>
                        <div class="card-body">
                        <div class="card-info-item">
                                <div class="card-info-value">الكلية</div>
                                <div class="card-info-label"> {{ $depart->id_coll }}</div>
                        ...
                        <div class="card-footer">
                            ...
                            <div class="card-actions">
                                ...
                                <button class="card-action-btn edit" onclick="editEmployee({{ $depart->id }},
                               '{{ $depart->depart_name }}','{{ $depart->id_coll }}')">
                               <i class="fas fa-edit" style=" font-size:15px;"></i>
                               </button>
                            </div>
    ...
    <div class="modal" id="employeeModal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title" id="modalTitle">إضافة تخصص جديد</h2>
            ...
            <div class="modal-body">
                ...
                @csrf
                            ...
                            <div class="form-group">
                                <label class="form-label" for="name">اسم التخصص </label>
                                <input type="text" class="form-input" name="depart_name"  placeholder="أدخل اسم التخصص"  required>
                            </div>
                            <div class="form-group">
        <label class="form-label" for="status">اسم الكلية</label>
        <select class="form-select" name="id_coll" required>
            <option value="">اختر الكلية</option>
            @foreach ($colloges as $colloge)
            <option value="{{ $colloge->id }}">{{ $colloge->coll_name }}</option>
        @endforeach
...
<div class="modall" id="employeeModall" >
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title" >تعديل تخصص </h2>
                <button class="close-btn" onclick="closeModal()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            ...
            <form id="editUniversityForm" action="{{ route('departs.update', 0) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="form-group">
    <label class="form-label" for="name">اسم التخصص </label>
    <input type="text" class="form-input" id="depart_name" name="depart_name" placeholder="" value="{{ $depart->depart_name }}" required>
    </div>
    <div class="form-group">
                            <label class="form-label" for="name" >اسم الكلية </label>
                            <select name="id_coll" id="id_coll"  required>
                @foreach ($colloges as $colloge)
                    <option value="{{ $colloge->id }}">{{ $colloge->coll_name }}</option>
                @endforeach
            </select>                            </div>
    ...
    <div class="modal" id="deleteModal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="hidden" id="id_university" >
            </div>
            <div class="modal-footer">
    ...
    function editEmployee(id,depart_name,id_coll) {
    document.getElementById('id').value = id;
    document.getElementById('depart_name').value = depart_name;
    document.getElementById('id_coll').value = id_coll;

    // تحديث action URL للنموذج
    document.getElementById('editUniversityForm').action = "{{ route('departs.update', '') }}" + '/' + id;
